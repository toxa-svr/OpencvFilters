...
{
    // Create scene and set QGraphicsScene for this QGraphicsView
    ...
    if (tmpConnection != 0) {
            ...
            startConnectors.first() != endConnectors.first())
		{
            ...
            if (!((startConnector->connectorType() == NodePort::In && endConnector->connectorType() == NodePort::In) || (startConnector->connectorType() == NodePort::Out && endConnector->connectorType() == NodePort::Out)))
			{
                ...
                arrow->setColor(settings.connectionLineColor);
...
QGraphicsItem * NodeEditorScene::addNode()
{
    NodeItem *newNode = createGraphNode();
    return newNode;
}

QGraphicsItem * NodeEditorScene::addNode(const QWidget &widget)
{
    NodeItem *newNode = createGraphNode();
...
QGraphicsItem * NodeEditorScene::addNode(const QString &title, const QString &smallTitle, const QPixmap &icon, const QWidget &widget, const bool isCollapsed)
{
    NodeItem *newNode = createGraphNode();
...
NodeItem * NodeEditorScene::createGraphNode()
{
...
QGraphicsItem * NodeEditorScene::addPort()
{
...
QGraphicsItem * NodeEditorScene::addConnection()
{
    return nullptr;
}
